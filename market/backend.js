module.exports = function (config) {
  if (config.backend) {
    config.cronjobs.recreateNpcOrders = [14400, () => basicNPCOrders(config)]
  }
}
  ...
  let ps = terminals.map(async terminal => {
    ...
    Object.keys(market.buyPrices).forEach(mineral => {
      ...
      if (market.priceMode === 'fixed') {
        ...
        let amount = Math.floor(market.buyAmount * (Math.random() * market.randomVariance + (market.randomVariance / 2)))/mineral.length
