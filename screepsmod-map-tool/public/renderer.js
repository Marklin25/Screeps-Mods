...
async function autoGen(sizex, sizey, generate3X3) {
  ...
  for (let ry = -hh * 10; ry <= hh * 10; ry++) {
    ...
    for (let x = startx; x < endx; x++) {
      ...
      if (!rooms.has(r) && (!generate3X3 || (x < 10 && y < 10))) {
        rooms.add(r)
      }
    }
  }
  ...
  for (const roomName of q1) {
    ps.push(gen(roomName, generate3X3).then(update))
  }
